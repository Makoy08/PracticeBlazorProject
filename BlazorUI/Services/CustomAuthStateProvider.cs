...
namespace BlazorUI.Services
{
    public class CustomAuthStateProvider : AuthenticationStateProvider
    {
        ...
        {
            //this method is sample only widmark ok?? the first line debug for non auth the 2nd method that commented is authenticated

            //var user = new ClaimsPrincipal(new ClaimsIdentity()); //non authenticated user 

            //var claims = new List<Claim> { new Claim(ClaimTypes.Name, "John") }; //authentidacted user
        ...
        public async Task<FormResult> LoginAsync(string email, string password)
        {
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    //need to refresh auth state the token ok widmark?
                ...
                {
                    return new FormResult { Succeeded = false, Errors = ["Bad Email or Passowrd"] };
                }
            }
            catch { }
        ...
        public void Logout()
        {
            //delete from localstorage once logout
